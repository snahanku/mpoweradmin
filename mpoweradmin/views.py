...
from mpoweradmin.models import Post_Seminars, Test
def create_seminars(request ,pk):
    pk=pk
    if request.method=="POST":
           seminar_title= request.POST.get("seminar_title")
           seminar_date=request.POST.get("seminar_date")
           seminar_time=request.POST.get("seminar_time")
           seminar_venue=request.POST.get("seminar_venue")
           venue_location=request.POST.get("venue_location")
           venue_offline_location=str(request.POST.get("offline_location"))
           technology=request.POST.get("technology")
           host_company=request.POST.get("company_host")
           links=request.POST.get("links")
           print("every thing is fine")

           data=Post_Seminars.objects.create(
                 Seminar_title=seminar_title,
                 Seminar_Date=seminar_date,
                 Seminar_Time=seminar_time,
                 Seminar_Venue=seminar_venue,
                 Venue_location=venue_location,
                 Venue_offline_location=venue_offline_location,
                 Technology_tags=technology,
                 Host_Company=host_company,
                Links=links ,
                venue_offline=venue_offline_location)
           data.save()
           return redirect("mpoweradmin:cdashboard",pk=pk)
           #return render(request,"data.html",{"data": Post_Seminars.objects.all})
          
           
        #data.save()

    return render(request ,"create_seminars.html" )
...
def create_test(request , pk):
    ...
    if request.method =="POST":
          ...
          data.save()
          return redirect("mpoweradmin:cdashboard",pk=pk)
          #return render(request,"data.html",{"testdata":Test.objects.all()})
